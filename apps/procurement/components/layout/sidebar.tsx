...
export function Sidebar() {
  ...
  const isActive = (item: NavItem) => {
    if (item.href === "/") {
      return currentPath() === "/" || currentPath().startsWith("/pr")
    }
      ...
      <div class="flex items-center gap-3 px-4 h-14 border-b border-gray-100 flex-shrink-0">
        ...
        <Show when={!collapsed()}>
          <div class="overflow-hidden">
            <p class="text-sm font-semibold text-gray-900 truncate">Procurement</p>
            <p class="text-[11px] text-gray-500 truncate">Requests & Orders</p>
          </div>
        </Show>
      </div>
