...
export interface CheckVoucherFormValues {
  voucherNo?: string
  voucherDate: string
  payee: string
  address?: string
...
interface CheckVoucherFormProps {
  initialValues: CheckVoucherFormValues
  title: string
  subtitle: string
  submitLabel: string
  voucherNumberRequired?: boolean
  warning?: string
  pending: boolean
  error?: Error | null
  onSubmit: (values: CheckVoucherFormValues) => void
  onCancel: () => void
}
...
function paymentDrafts(lines: CheckVoucherPaymentLine[]) {
  return lines.length
    ? lines.map(line => ({ description: line.description, amount: amountInput(line.amount) }))
    : [blankPayment()]
}

function accountDrafts(lines: CheckVoucherLine[]) {
  return lines.length
    ? lines.map(line => ({ account: line.account, amount: amountInput(line.amount) }))
    : [blankAccount()]
}
...
export function CheckVoucherForm(props: CheckVoucherFormProps) {
  const initial = props.initialValues
  const [voucherNo, setVoucherNo] = createSignal(initial.voucherNo ?? "")
  const [voucherDate, setVoucherDate] = createSignal(initial.voucherDate)
  const [payee, setPayee] = createSignal(initial.payee)
  const [address, setAddress] = createSignal(initial.address ?? "")
  const [bankName, setBankName] = createSignal(initial.bankName)
  const [checkNo, setCheckNo] = createSignal(initial.checkNo ?? "")
  const [preparedBy, setPreparedBy] = createSignal(initial.preparedBy ?? "")
  const [approvedBy, setApprovedBy] = createSignal(initial.approvedBy ?? "")
  const [receivedBy, setReceivedBy] = createSignal(initial.receivedBy ?? "")
  const [paymentLines, setPaymentLines] = createSignal<PaymentDraft[]>(
    paymentDrafts(initial.paymentLines)
  )
  const [debitLines, setDebitLines] = createSignal<AccountDraft[]>(
    accountDrafts(initial.debitLines)
  )
  const [creditLines, setCreditLines] = createSignal<AccountDraft[]>(
    accountDrafts(initial.creditLines)
  )
  ...
  const validate = () => {
    ...
    if (props.voucherNumberRequired && !voucherNo().trim())
      next.voucherNo = "Voucher number is required"
    ...
    <div class="px-4 py-5 sm:px-8 sm:py-8 lg:px-12">
      <div class="mx-auto max-w-6xl">
        <div class="mb-6 flex items-start gap-3 sm:mb-8 sm:items-center">
          <BackLink variant="icon" label="Back to check vouchers" onClick={props.onCancel} />
          <div>
            <h1 class="text-xl font-semibold text-foreground sm:text-2xl">{props.title}</h1>
            ...
            <div class="space-y-6">
              <section class="space-y-4 rounded-lg border border-border bg-surface p-4 sm:p-6">
                <Show when={props.warning}>
                  {warning => (
                    <div class="rounded-lg border border-accent/30 bg-accent/5 px-4 py-3 text-sm text-foreground">
                      {warning()}
                    </div>
                  )}
                </Show>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Input
                    ...
                    placeholder="0003"
                    hint={
                      props.voucherNumberRequired
                        ? "Required; changes are recorded in the audit log"
                        : "Optional; leave blank to assign automatically"
                    }
                  />
                  <DateInput
                    label="Date"
