import { ConfirmDialog, Modal, ModalFooter } from "@ark/ui"
...
export function ManageVenuesModal(props: ManageVenuesModalProps) {
  ...
  const [deletingVenue, setDeletingVenue] = createSignal<Venue | null>(null)
  ...
  const handleDelete = () => {
    const venue = deletingVenue()
    if (!venue) return
    deleteMutation.mutate(venue.id, { onSuccess: () => setDeletingVenue(null) })
  }
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={
          <span>
            Remove <span class="font-medium">{deletingVenue()?.name}</span>? Existing batches will
            keep this venue name.
          </span>
        }
