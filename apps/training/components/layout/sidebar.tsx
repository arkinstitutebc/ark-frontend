...
export function Sidebar() {
  ...
  const isActive = (item: NavItem) => {
    if (item.href === "/") {
      return currentPath() === "/" || currentPath().startsWith("/batch")
    }
    ...
    <div class="flex flex-col h-full bg-white border-r border-gray-200">
      ...
      <div class="flex items-center gap-3 px-4 h-14 border-b border-gray-100 flex-shrink-0">
        <div class="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
          <GraduationCap class="w-4.5 h-4.5 text-primary" />
        </div>
        <Show when={!collapsed()}>
          <div class="overflow-hidden">
            <p class="text-sm font-semibold text-gray-900 truncate">Training</p>
            <p class="text-[11px] text-gray-500 truncate">Batches & Enrollments</p>
          </div>
        </Show>
      </div>
